...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.companies.update", [$company->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="description">{{ trans('cruds.company.fields.description') }}</label>
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description" required>{{ old('description', $company->description) }}</textarea>
            ...
            <div class="form-group">
                <label class="required" for="size">{{ trans('cruds.company.fields.size') }}</label>
                <input class="form-control {{ $errors->has('size') ? 'is-invalid' : '' }}" type="text" name="size" id="size" value="{{ old('size', $company->size) }}" required>
                @if($errors->has('size'))
                    <span class="text-danger">{{ $errors->first('size') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.company.fields.size_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="industries">{{ trans('cruds.company.fields.industries') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('industries') ? 'is-invalid' : '' }}" name="industries[]" id="industries" multiple required>
                    @foreach($industries as $id => $industry)
                        <option value="{{ $id }}" {{ (in_array($id, old('industries', [])) || $company->industries->contains($id)) ? 'selected' : '' }}>{{ $industry }}</option>
                    @endforeach
                </select>
                @if($errors->has('industries'))
                    <span class="text-danger">{{ $errors->first('industries') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.company.fields.industries_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="since">{{ trans('cruds.company.fields.since') }}</label>
                <input class="form-control date {{ $errors->has('since') ? 'is-invalid' : '' }}" type="text" name="since" id="since" value="{{ old('since', $company->since) }}">
                @if($errors->has('since'))
                    <span class="text-danger">{{ $errors->first('since') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.company.fields.since_helper') }}</span>
            </div>
            <div class="form-group">
