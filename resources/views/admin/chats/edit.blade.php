        ...
        <form method="POST" action="{{ route("admin.chats.update", [$chat->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_read') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_read" id="is_read" value="1" {{ $chat->is_read || old('is_read', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="is_read">{{ trans('cruds.chat.fields.is_read') }}</label>
                </div>
