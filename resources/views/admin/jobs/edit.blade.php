...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.jobs.update", [$job->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="working_hour">{{ trans('cruds.job.fields.working_hour') }}</label>
                <input class="form-control {{ $errors->has('working_hour') ? 'is-invalid' : '' }}" type="text" name="working_hour" id="working_hour" value="{{ old('working_hour', $job->working_hour) }}" required>
                @if($errors->has('working_hour'))
                    <span class="text-danger">{{ $errors->first('working_hour') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.job.fields.working_hour_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="closed_date">{{ trans('cruds.job.fields.closed_date') }}</label>
                <input class="form-control date {{ $errors->has('closed_date') ? 'is-invalid' : '' }}" type="text" name="closed_date" id="closed_date" value="{{ old('closed_date', $job->closed_date) }}">
            ...
            <div class="form-group">
                <label for="responsibility">{{ trans('cruds.job.fields.responsibility') }}</label>
                <textarea class="form-control {{ $errors->has('responsibility') ? 'is-invalid' : '' }}" name="responsibility" id="responsibility">{{ old('responsibility', $job->responsibility) }}</textarea>
                @if($errors->has('responsibility'))
                    <span class="text-danger">{{ $errors->first('responsibility') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.job.fields.responsibility_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="requirement">{{ trans('cruds.job.fields.requirement') }}</label>
                <textarea class="form-control {{ $errors->has('requirement') ? 'is-invalid' : '' }}" name="requirement" id="requirement">{{ old('requirement', $job->requirement) }}</textarea>
            ...
            <div class="form-group">
                <label for="benefits">{{ trans('cruds.job.fields.benefits') }}</label>
                <textarea class="form-control {{ $errors->has('benefits') ? 'is-invalid' : '' }}" name="benefits" id="benefits">{{ old('benefits', $job->benefits) }}</textarea>
                @if($errors->has('benefits'))
                    <span class="text-danger">{{ $errors->first('benefits') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.job.fields.benefits_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="category_id">{{ trans('cruds.job.fields.category') }}</label>
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id" required>
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $job->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
