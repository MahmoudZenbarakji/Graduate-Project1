...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.skills.update", [$skill->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="description">{{ trans('cruds.skill.fields.description') }}</label>
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $skill->description) }}</textarea>
                @if($errors->has('description'))
                    <span class="text-danger">{{ $errors->first('description') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.skill.fields.description_helper') }}</span>
            </div>
            <div class="form-group">
